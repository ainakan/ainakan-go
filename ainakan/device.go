...
// Device represents Device struct from ainakan-core
type Device struct {
	device *C.AinakanDevice
}

// ID will return the ID of the device.
func (d *Device) ID() string {
	if d.device != nil {
		return C.GoString(C.ainakan_device_get_id(d.device))
	}
	return ""
}
...
func (d *Device) Name() string {
	if d.device != nil {
		return C.GoString(C.ainakan_device_get_name(d.device))
	}
	return ""
}
...
func (d *Device) DeviceIcon() any {
	if d.device != nil {
		icon := C.ainakan_device_get_icon(d.device)
...
func (d *Device) DeviceType() DeviceType {
	if d.device != nil {
		fdt := C.ainakan_device_get_dtype(d.device)
...
func (d *Device) Bus() *Bus {
	if d.device != nil {
		bus := C.ainakan_device_get_bus(d.device)
		return &Bus{
			bus: bus,
		}
	}
...
func (d *Device) IsLost() bool {
	if d.device != nil {
		lost := C.ainakan_device_is_lost(d.device)
		return int(lost) == 1
	}
	return false
}
...
// This function will properly handle cancelling the ainakan operation.
...
//	// or WithCancel
//
//	cancel := ainakan.NewCancellable()
//	params, err := device.Params(ainakan.WithCancel(c))
//
//	// ...
//
//	cancel.Cancel()
...
func (d *Device) params(opts options) (map[string]any, error) {
	...
	var err *C.GError
	ht := C.ainakan_device_query_system_parameters_sync(d.device, opts.cancellable, &err)
	if err != nil {
...
func (d *Device) FrontmostApplication(scope Scope) (*Application, error) {
	if d.device != nil {
		...
		sc := C.AinakanScope(scope)
		queryOpts := C.ainakan_frontmost_query_options_new()
		C.ainakan_frontmost_query_options_set_scope(queryOpts, sc)
		app.application = C.ainakan_device_get_frontmost_application_sync(d.device,
			queryOpts,
			nil,
			&err)
		if err != nil {
...
// Example:
//
//	apps, err := device.EnumerateApplications("", ainakan.ScopeFull)
//
//	// or providing the option to cancel
//
//	cancel := ainakan.NewCancellable()
//	apps, err := device.EnumerateApplications("", ainakan.ScopeFull, ainakan.WithCancel(c))
//
//	// ...
//
//	cancel.Cancel()
...
func (d *Device) enumerateApplications(identifier string, scope Scope, opts options) ([]*Application, error) {
	...
	queryOpts := C.ainakan_application_query_options_new()
	C.ainakan_application_query_options_set_scope(queryOpts, C.AinakanScope(scope))

	if identifier != "" {
		identifierC := C.CString(identifier)
		defer C.free(unsafe.Pointer(identifierC))
		C.ainakan_application_query_options_select_identifier(queryOpts, identifierC)
	}

	var err *C.GError
	appList := C.ainakan_device_enumerate_applications_sync(d.device, queryOpts, opts.cancellable, &err)
	if err != nil {
		return nil, handleGError(err)
	}

	appListSize := int(C.ainakan_application_list_size(appList))
	apps := make([]*Application, appListSize)

	for i := 0; i < appListSize; i++ {
		app := C.ainakan_application_list_get(appList, C.gint(i))
	...
	clean(unsafe.Pointer(queryOpts), unrefAinakan)
	clean(unsafe.Pointer(appList), unrefAinakan)
...
func (d *Device) ProcessByPID(pid int, scope Scope) (*Process, error) {
	...
	opts := C.ainakan_process_match_options_new()
	C.ainakan_process_match_options_set_timeout(opts, C.gint(defaultProcessTimeout))
	C.ainakan_process_match_options_set_scope(opts, C.AinakanScope(scope))
	defer clean(unsafe.Pointer(opts), unrefAinakan)

	var err *C.GError
	proc := C.ainakan_device_get_process_by_pid_sync(d.device, C.guint(pid), opts, nil, &err)
...
func (d *Device) ProcessByName(name string, scope Scope) (*Process, error) {
	...
	var err *C.GError
	proc := C.ainakan_device_get_process_by_name_sync(d.device, nameC, opts, nil, &err)
	return &Process{proc}, handleGError(err)
}
...
func (d *Device) FindProcessByPID(pid int, scope Scope) (*Process, error) {
	...
	var err *C.GError
	proc := C.ainakan_device_find_process_by_pid_sync(d.device, C.guint(pid), opts, nil, &err)
	return &Process{proc}, handleGError(err)
}
...
func (d *Device) FindProcessByName(name string, scope Scope) (*Process, error) {
	...
	var err *C.GError
	proc := C.ainakan_device_find_process_by_name_sync(d.device, nameC, opts, nil, &err)
	return &Process{proc}, handleGError(err)
}
...
func (d *Device) EnumerateProcesses(scope Scope) ([]*Process, error) {
	if d.device != nil {
		opts := C.ainakan_process_query_options_new()
		C.ainakan_process_query_options_set_scope(opts, C.AinakanScope(scope))
		defer clean(unsafe.Pointer(opts), unrefAinakan)

		var err *C.GError
		procList := C.ainakan_device_enumerate_processes_sync(d.device, opts, nil, &err)
		if err != nil {
			return nil, handleGError(err)
		}

		procListSize := int(C.ainakan_process_list_size(procList))
		procs := make([]*Process, procListSize)

		for i := 0; i < procListSize; i++ {
			proc := C.ainakan_process_list_get(procList, C.gint(i))
			procs[i] = &Process{proc}
		}

		clean(unsafe.Pointer(procList), unrefAinakan)
		return procs, nil
	}
...
func (d *Device) EnableSpawnGating() error {
	...
	var err *C.GError
	C.ainakan_device_enable_spawn_gating_sync(d.device, nil, &err)
...
func (d *Device) DisableSpawnGating() error {
	...
	var err *C.GError
	C.ainakan_device_disable_spawn_gating_sync(d.device, nil, &err)
	return handleGError(err)
}
...
func (d *Device) EnumeratePendingSpawn() ([]*Spawn, error) {
	if d.device != nil {
		var err *C.GError
		spawnList := C.ainakan_device_enumerate_pending_spawn_sync(d.device, nil, &err)
		if err != nil {
			return nil, handleGError(err)
		}

		spawnListSize := int(C.ainakan_spawn_list_size(spawnList))
		spawns := make([]*Spawn, spawnListSize)

		for i := 0; i < spawnListSize; i++ {
			spawn := C.ainakan_spawn_list_get(spawnList, C.gint(i))
			spawns[i] = &Spawn{spawn}
		}

		clean(unsafe.Pointer(spawnList), unrefAinakan)
		return spawns, nil
	}
...
func (d *Device) EnumeratePendingChildren() ([]*Child, error) {
	if d.device != nil {
		var err *C.GError
		childList := C.ainakan_device_enumerate_pending_children_sync(d.device, nil, &err)
		if err != nil {
			return nil, handleGError(err)
		}

		childListSize := int(C.ainakan_child_list_size(childList))
		children := make([]*Child, childListSize)

		for i := 0; i < childListSize; i++ {
			child := C.ainakan_child_list_get(childList, C.gint(i))
			children[i] = &Child{child}
		}

		clean(unsafe.Pointer(childList), unrefAinakan)
		return children, nil
	}
...
func (d *Device) Spawn(name string, opts *SpawnOptions) (int, error) {
	...
	var opt *C.AinakanSpawnOptions = nil
	if opts != nil {
		opt = opts.opts
	}
	defer clean(unsafe.Pointer(opt), unrefAinakan)

	nameC := C.CString(name)
	defer C.free(unsafe.Pointer(nameC))

	var err *C.GError
	pid := C.ainakan_device_spawn_sync(d.device, nameC, opt, nil, &err)
...
func (d *Device) Input(pid int, data []byte) error {
	...
	var err *C.GError
	C.ainakan_device_input_sync(d.device, C.guint(pid), gBytesData, nil, &err)
...
func (d *Device) Resume(pid int) error {
	...
	var err *C.GError
	C.ainakan_device_resume_sync(d.device, C.guint(pid), nil, &err)
	return handleGError(err)
}
...
func (d *Device) Kill(pid int) error {
	...
	var err *C.GError
	C.ainakan_device_kill_sync(d.device, C.guint(pid), nil, &err)
	return handleGError(err)
}
...
func (d *Device) attach(val any, sessionOpts *SessionOptions, opts options) (*Session, error) {
	...
	var opt *C.AinakanSessionOptions = nil
	if sessionOpts != nil {
	...
	var err *C.GError
	s := C.ainakan_device_attach_sync(d.device, C.guint(pid), opt, opts.cancellable, &err)
...
func (d *Device) InjectLibraryFile(target any, path, entrypoint, data string) (uint, error) {
	...
	var err *C.GError
	id := C.ainakan_device_inject_library_file_sync(d.device,
		C.guint(pid),
		pathC,
		entrypointC,
		dataC,
...
func (d *Device) InjectLibraryBlob(target any, byteData []byte, entrypoint, data string) (uint, error) {
	...
	var err *C.GError
	id := C.ainakan_device_inject_library_blob_sync(d.device,
		C.guint(pid),
		gBytesData,
		entrypointC,
		dataC,
...
func (d *Device) OpenChannel(address string) (*IOStream, error) {
	...
	var err *C.GError
	stream := C.ainakan_device_open_channel_sync(d.device, addressC, nil, &err)
...
func (d *Device) OpenService(address string) (*Service, error) {
	...
	var err *C.GError
	svc := C.ainakan_device_open_service_sync(d.device, addrC, nil, &err)
...
func (d *Device) Clean() {
	if d.device != nil {
		clean(unsafe.Pointer(d.device), unrefAinakan)
	}
}
...
//   - "spawn_added" with callback as func(spawn *ainakan.Spawn) {}
//   - "spawn_removed" with callback as func(spawn *ainakan.Spawn) {}
//   - "child_added" with callback as func(child *ainakan.Child) {}
//   - "child_removed" with callback as func(child *ainakan.Child) {}
//   - "process_crashed" with callback as func(crash *ainakan.Crash) {}
