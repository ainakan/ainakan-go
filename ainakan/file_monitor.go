...
func NewFileMonitor(path string) *FileMonitor {
	...
	m := C.ainakan_file_monitor_new(pathC)
...
func (mon *FileMonitor) Path() string {
	return C.GoString(C.ainakan_file_monitor_get_path(mon.fm))
}

// Enable enables file monitoring.
func (mon *FileMonitor) Enable() error {
	var err *C.GError
	C.ainakan_file_monitor_enable_sync(mon.fm, nil, &err)
...
func (mon *FileMonitor) Disable() error {
	var err *C.GError
	C.ainakan_file_monitor_disable_sync(mon.fm, nil, &err)
	return handleGError(err)
}

// Clean will clean the resources held by the file monitor.
func (mon *FileMonitor) Clean() {
	clean(unsafe.Pointer(mon.fm), unrefAinakan)
}
