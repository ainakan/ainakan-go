...
func NewSnapshotOptions(warmupScript string, rt ScriptRuntime) *SnapshotOptions {
	opts := C.ainakan_snapshot_options_new()
	warmupScriptC := C.CString(warmupScript)
	defer C.free(unsafe.Pointer(warmupScriptC))

	C.ainakan_snapshot_options_set_warmup_script(opts, warmupScriptC)
	C.ainakan_snapshot_options_set_runtime(opts, C.AinakanScriptRuntime(rt))
...
func (s *SnapshotOptions) WarmupScript() string {
	return C.GoString(C.ainakan_snapshot_options_get_warmup_script(s.opts))
}

// Runtime returns the runtime used to create the script options.
func (s *SnapshotOptions) Runtime() ScriptRuntime {
	return ScriptRuntime(int(C.ainakan_snapshot_options_get_runtime(s.opts)))
}
