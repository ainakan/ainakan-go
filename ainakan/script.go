...
func (s *Script) IsDestroyed() bool {
	destroyed := C.ainakan_script_is_destroyed(s.sc)
...
func (s *Script) Load() error {
	...
	var err *C.GError
	C.ainakan_script_load_sync(s.sc, nil, &err)
...
func (s *Script) Unload() error {
	var err *C.GError
	C.ainakan_script_unload_sync(s.sc, nil, &err)
	return handleGError(err)
}
...
func (s *Script) Eternalize() error {
	var err *C.GError
	C.ainakan_script_eternalize_sync(s.sc, nil, &err)
	return handleGError(err)
}
...
func (s *Script) Post(jsonString string, data []byte) {
	...
	if len(data) > 0 {
		...
		C.ainakan_script_post(s.sc, jsonStringC, gBytesData)
		runtime.KeepAlive(gBytesData)
	} else {
		C.ainakan_script_post(s.sc, jsonStringC, nil)
	}
}
...
func (s *Script) EnableDebugger(port uint16) error {
	var err *C.GError
	C.ainakan_script_enable_debugger_sync(s.sc, C.guint16(port), nil, &err)
...
func (s *Script) DisableDebugger() error {
	var err *C.GError
	C.ainakan_script_disable_debugger_sync(s.sc, nil, &err)
	return handleGError(err)
}
...
func (s *Script) Clean() {
	clean(unsafe.Pointer(s.sc), unrefAinakan)
}
...
func (s *Script) hijackFn(message string, data []byte) {
	if strings.Contains(message, "ainakan:rpc") {
