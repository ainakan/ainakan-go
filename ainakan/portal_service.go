...
func NewPortal(clusterParams, controlParams *EndpointParameters) *Portal {
	p := C.ainakan_portal_service_new(clusterParams.params, controlParams.params)
...
func (p *Portal) Device() DeviceInt {
	dev := C.ainakan_portal_service_get_device(p.portal)
	return &Device{dev}
}

// ClusterParams returns the cluster parameters for the portal.
func (p *Portal) ClusterParams() *EndpointParameters {
	params := C.ainakan_portal_service_get_cluster_params(p.portal)
...
func (p *Portal) ControlParams() *EndpointParameters {
	params := C.ainakan_portal_service_get_control_params(p.portal)
	return &EndpointParameters{params}
}
...
func (p *Portal) Start() error {
	var err *C.GError
	C.ainakan_portal_service_start_sync(p.portal, nil, &err)
...
func (p *Portal) Stop() error {
	var err *C.GError
	C.ainakan_portal_service_stop_sync(p.portal, nil, &err)
	return handleGError(err)
}

// Kick kicks the connection with connectionID provided.
func (p *Portal) Kick(connectionID uint) {
	C.ainakan_portal_service_kick(p.portal, C.guint(connectionID))
}
...
func (p *Portal) Post(connectionID uint, json string, data []byte) {
	...
	C.ainakan_portal_service_post(p.portal, C.guint(connectionID), jsonC, gBytesData)
...
func (p *Portal) Narrowcast(tag, json string, data []byte) {
	...
	C.ainakan_portal_service_narrowcast(p.portal, tagC, jsonC, gBytesData)
	runtime.KeepAlive(gBytesData)
}
...
func (p *Portal) Broadcast(json string, data []byte) {
	...
	C.ainakan_portal_service_broadcast(p.portal, jsonC, gBytesData)
	runtime.KeepAlive(gBytesData)
}
...
func (p *Portal) EnumerateTags(connectionID uint) []string {
	var length C.gint
	tagsC := C.ainakan_portal_service_enumerate_tags(
		p.portal,
...
func (p *Portal) TagConnection(connectionID uint, tag string) {
	...
	C.ainakan_portal_service_tag(p.portal, C.guint(connectionID), tagC)
}
...
func (p *Portal) UntagConnection(connectionID uint, tag string) {
	...
	C.ainakan_portal_service_untag(p.portal, C.guint(connectionID), tagC)
}

// Clean will clean the resources held by the ainakan.
func (p *Portal) Clean() {
	clean(unsafe.Pointer(p.portal), unrefAinakan)
}
...
//   - "node_connected" with callback as func(connId uint, addr *ainakan.Address) {}
//   - "node_joined" with callback as func(connId uint, app *ainakan.Application) {}
//   - "node_left" with callback as func(connId uint, app *ainakan.Application) {}
//   - "node_disconnected" with callback as func(connId uint, addr *ainakan.Address) {}
//   - "controller_connected" with callback as func(connId uint, addr *ainakan.Address) {}
//   - "controller_disconnected" with callback as func(connId uint, addr *ainakan.Address) {}
